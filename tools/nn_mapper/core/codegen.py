"""Emit mapper output in two forms:

  1. devices.json (to_devices_json/write_devices_json) -- matches
     tools/nn_setup/device_manifest.py's schema EXACTLY, so it can be
     passed straight to `python setup_tool.py --config devices.json`.
     This is the real target format, not a placeholder: setup_tool.py
     already consumes it as-is.
  2. A direct C++ header per node (to_cpp_header) matching NNNodeConfig's
...
since 0 is the real, meaningful virtual-input layer id here.
